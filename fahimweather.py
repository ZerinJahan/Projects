...
dataset = pd.read_csv('F:/weatherData2006.csv')
...
ct = ColumnTransformer([("Precip Type", OneHotEncoder(), [1])], remainder = 'passthrough')
...
y = label_binarize(y, classes=np.arange(0,75))
n_classes = 75
...
classifier.add(Dense(output_dim = 83, init = 'uniform', activation = 'relu', input_dim = 14))
...
classifier.fit(x_train, y_train, batch_size = 300, epochs = 1000)
